...
pub fn decode_thumb_narrow(hword: u16, c: Context) -> u32 {
    ...
    return match hword >> 10 {
        ...
        0b11_0000..=0b11_0001 => {
            let mut base = tag::get_narrow(Opcode::Stm, c, hword & 0x7FF); // A7.7.159 T1
            if hword & 0xFF == 0 {
        ...
        0b11_1000..=0b11_1001 => {
            let mut base = tag::get_narrow(Opcode::Branch, c, hword & 0x7FF); // A7.7.12 T2
...
fn id_shift_add_sub_move_cmp(hword: u16, c: Context) -> u32 {
    ...
    let rd = hword & 0x7;
    let rm = (hword >> 3) & 0x7;
    let imm5 = (hword >> 6) & 0x1F;
    let imm6_r = if imm5 == 0 { 32 } else { imm5 };

    return match hword >> 9 {
        0b00000..=0b00011 => {
            if (hword >> 6) == 0 {
                let mut base = tag::get_narrow(Opcode::MovReg, c, rd | rm << 4 | 1 << 8); // A7.7.77 T2
                if c.it_pos == ItPos::None {
                    base = tag::as_unpred_it(base);
                }
                base
            } else {
                tag::get_narrow(Opcode::LslImm, c, rd | rm << 3 | imm5 << 6) // A7.7.68 T1
            }
        }
        0b00100..=0b00111 => tag::get_narrow(Opcode::LsrImm, c, rd | rm << 3 | imm6_r << 6), // A7.7.70 T1
        0b01000..=0b01011 => tag::get_narrow(Opcode::AsrImm, c, rd | rm << 3 | imm6_r << 6), // A7.7.10 T1
        0b01100 => {
        ...
        0b01101 => tag::get_narrow(Opcode::SubReg, c, hword & 0x1FF), // A7.7.175 T1
...
fn id_data_processing(hword: u16, c: Context) -> u32 {
    ...
    let opcode = match (hword >> 6) & 0xF {
        ...
        0b0110 => Opcode::SbcReg, // A7.7.125 T1
...
fn id_special_data_branch(hword: u16, c: Context) -> u32 {
    ...
    return match (hword >> 6) & 0xF {
        0b0000..=0b0011 => {
            let mut base = tag::get_narrow(Opcode::AddReg, c, rd | (rm << 4) | (rd << 8)); // A7.7.4 T2
            if rd == 15 {
                if rm == (15 << 4) {
                    base = tag::as_unpred(base);
                }
        ...
        0b0101..=0b0111 => {
            let mut base = tag::get_narrow(Opcode::CmpReg, c, rd | (rm << 4)); // A7.7.28 T2
        ...
        0b1000..=0b1011 => {
            let mut base = tag::get_narrow(Opcode::MovReg, c, rd | rm << 4); // A7.7.77 T1
        ...
        0b1100..=0b1101 => {
            let mut base = tag::get_narrow(Opcode::Bx, c, rm); // A7.7.20 T1
            if hword & 0b111 != 0 {
                base = tag::as_unpred(base);
            }
            if c.it_pos == ItPos::Within {
        ...
        0b1110..=0b1111 => {
            let mut base = tag::get_narrow(Opcode::Blx, c, rm); // A7.7.19 T1
            if hword & 0b111 != 0 {
                base = tag::as_unpred(base);
            }
            if c.it_pos == ItPos::Within {
...
fn id_misc(hword: u16, c: Context) -> u32 {
    ...
    return match (hword >> 5) & 0x7F {
        ...
        0b101_1000..=0b101_1111 => {
            let mut base = tag::get_narrow(Opcode::Cbz, c, (hword & (0x1F << 3)) >> 2 | (hword & (1 << 9)) >> 3 | (hword & 0b111) << 7 | (hword & (1 << 11)) >> 1); // A7.7.21 T1
            if c.it_pos != ItPos::None {
                base = tag::as_unpred_it(base)
            }
            base
...
fn id_conditional_branch_supc(hword: u16, c: Context) -> u32 {
    ...
    return match (hword >> 8) & 0b1111 {
        ...
        _ => {
            let mut base = tag::get_narrow(Opcode::BranchCond, c, hword & 0xFFF); // A7.7.12 T1
            if c.it_pos != ItPos::None {
                base = tag::as_unpred_it(base);
            }
            base
